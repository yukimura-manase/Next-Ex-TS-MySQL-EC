...
// ラーメンの商品情報を取得する！
app.get(`/api/products`, (req: Request, res: { json: (arg0: any) => void; })=> {

    console.log('get通信！ラーメンデータ取得！');
    //console.log(req);

    connection.query('select * from products;', (error: any, results: any)=> {
        // console.log(error);
        // console.log(results);
...
// ユーザーの新規登録
app.post(`/api/entry`, (req: { body: { name: string; user_image: string; password: string; mail: string; phoneNumber: string; addressNumber: string; address: string; point: number; likeItemList: JSON; }; }, res: { json: (arg0: string) => void; })=> {
    ...
    connection.query(`select * from users where name = '${name}';`, (error: any, results: any)=> {
        // console.log(error);
        // console.log(results);
...
// ユーザーのログイン
app.get(`/api/login`,(req: { query: { name: string; password: string; }; }, res: { json: (arg0: string) => void; })=> {
    ...
    connection.query(`select * from users where name = '${name}' and password = '${password}';`,(error: any, results: any)=> {
...
// ユーザーのカート情報を取得する！
app.post(`/api/cart/:user_id`, (req: { params: { user_id: string; }; }, res: { json: (arg0: string) => void; })=> {
...
app.post(`/api/newcart`, (req: { body: { cart_id: any; status: any; orderDate: any; userName: any; mail: any; addressNumber: any; address: any; phoneNumber: any; deliveryDate: any; deliveryTime: any; cartItemList: any; }; }, res: { json: (arg0: any) => void; })=> {
...
app.post(`/api/likeadd/:user_id`, (req: { params: { user_id: any; }; body: { likeadd: any; }; }, res: { send: (arg0: string) => void; })=>{
    console.log('お気に入り登録！');
    console.log(req.params);
    console.log(req.body);
    ...
    [req.params.user_id],
    (error: any, results: { likeItemList: string; }[])=> {
        console.log(error);
        console.log(results);
...
app.post(`/api/likeremove/:user_id`, (req: { body: { index: any; }; params: { user_id: any; }; }, res: { send: (arg0: string) => void; })=> {
    console.log('お気に入り削除！');
    // console.log(req.params);
    // console.log(req.body);
...
app.post(`/api/cartadd/:user_id`, (req: { params: { user_id: any; }; body: { cartadd: any; }; }, res: { send: (arg0: string) => void; })=>{
    ...
    [req.params.user_id],
    (error: any, results: { cartItemList: string; }[])=> {
        console.log(error);
        console.log(results);
...
app.post(`/api/cartremove/:user_id`, (req: { body: { index: any; }; params: { user_id: any; }; }, res: { send: (arg0: string) => void; })=> {
...
app.post(`/api/order/:user_id`, (req: { params: { user_id: any; }; body: { status: any; orderDate: any; userName: any; mail: any; addressNumber: any; address: any; phoneNumber: any; deliveryDate: any; deliveryTime: any; cartItemList: any; }; }, res: { send: (arg0: string) => void; })=> {
...
app.post(`/api/myinfo/update/:user_id`, (req: { body: { name: any; password: any; mail: any; phoneNumber: any; addressNumber: any; address: any; }; params: { user_id: any; }; }, res: { json: (arg0: { parseResult: any; message: string; }) => void; send: (arg0: string) => void; })=> {
    ...
    connection.query(`select * from users where name = '${name}' ; `,(error: any, results: string | any[])=> {
        console.log(error);
        console.log(results);
        ...
        if(results.length === 0){   // ①まずユーザー名の重複がないかを確認して、重複なければ(results空配列)、そのままupdate！
            ...
            (error: any, results: any)=> {
                ...
                (error: any, results: { likeItemList: any; }[])=> {
...
app.post(`/api/myinfo/delete/:user_id`,(req: { body: any; params: { user_id: any; }; }, res: { send: (arg0: string) => void; })=> {
...
app.post(`/api/session`, (req: { session: { userSessionId: any; name: any; }; body: { user_id: any; name: any; }; }, res: { json: (arg0: { parseUserData: any; cartList: any; }) => void; })=> {
    ...
    (error: any, results: { likeItemList: any; }[])=>{
        ...
        connection.query(`select * from carts where cart_id = '${userSessionId}' ;`,(error: any, results: any)=> {
            console.log(error);
            console.log(results);
...
app.post(`/api/point/:user_id`, (req: { body: { point: any; }; params: { user_id: any; }; }, res: { json: (arg0: any) => void; })=>{
...
app.use((err: any, req: any, res:any, next:any) => {
    ...
    res.status(err.stats || 500).send(`<html><body style=" text-align: center; " ><h1>APIサーバーにてエラーが発生しました！！</h1><img src="/ロボ玉2.jpg" alt="ロボ玉" style=" width: 900px; height: 600px; " /><p>${err.stack}</p></body></html>` );
});
